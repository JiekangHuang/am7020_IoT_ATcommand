...
void loop()
{
    ...
    if (millis() > pub_data_timer) {
        ...
        if (mqtt.publish(MQTT_TOPIC, String(millis()).c_str())) {
...
void reConnBroker(void)
{
    if (!mqtt.chkConnBroker()) {
        SerialMon.print(F("Connecting to "));
        SerialMon.print(MQTT_BROKER);
        SerialMon.print(F("..."));
        if (mqtt.connBroker(MQTT_BROKER, MQTT_PORT, MQTT_USERNAME, MQTT_PASSWORD, "MY_AM7020_TEST_MQTTID")) {
            SerialMon.println(F(" success"));
            SerialMon.print(F("subscribe: " MQTT_TOPIC));
            if (mqtt.subscribe(MQTT_TOPIC, callback)) {
                SerialMon.println(F(" success"));
            } else {
                SerialMon.println(F(" fail"));
            }
